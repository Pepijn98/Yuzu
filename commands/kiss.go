...
// IsOwnerOnly f
func (Kiss) IsOwnerOnly() bool {
	return false
}

// Help f
func (Kiss) Help() [2]string {
	return [2]string{"Give someone a kiss", "<@mention>"}
}

// Process f
func (Kiss) Process(ctx yuzu.Context) {
	...
	if mentionedUsers == 0 {
		_, er := ctx.Say("Please mention a member to kiss.")
		if er != nil {
			return
		}
		...
		// Do the stuff
		img, err := weeb.GetImage("kiss")
		if err != nil {
			functions.ReportError(ctx.Session, fmt.Sprintf("%s", err), "/commands/kiss.go")
		...
		embed := yuzu.NewEmbed(author + " kisses " + mentioned)
		embed.Image(img)
